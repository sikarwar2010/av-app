...
export const getContact = query({
    ...
    handler: async (ctx, args) => {
        ...
        const activities = await ctx.db
            .query("activities")
            .withIndex("by_related", (q) => q.eq("relatedTo", "contact").eq("relatedId", args.id))
            .order("desc")
            .take(10)
...
export const createContact = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.title?.toLowerCase().includes("ceo") || args.title?.toLowerCase().includes("cto")) aiScore += 15
