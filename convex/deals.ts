...
export const getDeals = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.search) {
            ...
            deals = deals.filter((deal) => deal.name.toLowerCase().includes(searchLower))
        }
...
export const getDealsByStage = query({
    ...
    handler: async (ctx) => {
        ...
        Object.keys(dealsByStage).forEach((stage) => {
            dealsByStage[stage].sort((a, b) => (b.aiProbability || 0) - (a.aiProbability || 0))
        })
...
export const getDeal = query({
    ...
    handler: async (ctx, args) => {
        ...
        const activities = await ctx.db
            .query("activities")
            .withIndex("by_related", (q) => q.eq("relatedTo", "deal").eq("relatedId", args.id))
            .order("desc")
            .take(10)
...
export const updateDeal = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (updates.amount && updates.amount !== deal.amount) {
            changes.push(`Amount: $${deal.amount?.toLocaleString()} → $${updates.amount.toLocaleString()}`)
        }
