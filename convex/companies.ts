    ...
    handler: async (ctx, args) => {
        ...
        const companiesWithData = await Promise.all(
            results.map(async (company) => {
                ...
                const totalDealValue = activeDeals.reduce((sum, deal) => sum + deal.amount, 0)
...
export const getCompany = query({
    ...
    handler: async (ctx, args) => {
        ...
        const activities = await ctx.db
            .query("activities")
            .withIndex("by_related", (q) => q.eq("relatedTo", "company").eq("relatedId", args.id))
            .order("desc")
            .take(20)
...
export const updateCompany = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (updates.annualRevenue !== undefined || updates.employees !== undefined) {
