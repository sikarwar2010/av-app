...
import { Brain, Target, Calendar, TrendingUp, Edit, Trash2 } from "lucide-react"
...
export function DealDetailSheet({ deal, open, onOpenChange }: DealDetailSheetProps) {
    ...
    const dealDetails = useQuery(api.deals.getDeal, deal ? { id: deal._id } : "skip")
    ...
    return (
        <>
                    ...
                    <SheetHeader>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <SheetTitle className="text-xl">{displayDeal.name}</SheetTitle>
                                    <SheetDescription>{displayDeal.company?.name}</SheetDescription>
                                </div>
                            </div>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold text-lg">${(displayDeal.amount / 1000).toFixed(0)}K</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-sm text-muted-foreground">Expected Close</span>
                                    <span className="text-sm">{new Date(displayDeal.closeDate).toLocaleDateString()}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                ...
                                {displayDeal.contact && (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-sm">{displayDeal.contact.name}</span>
                                    </div>
                                )}
                                <div className="flex items-center justify-between">
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <span
                                            className={`text-sm font-bold ${(displayDeal.aiProbability || displayDeal.probability) >= 70
                                                    ? "text-green-500"
                                                    : (displayDeal.aiProbability || displayDeal.probability) >= 40
                                                        ? "text-yellow-500"
                                                        : "text-red-500"
                                                }`}
                                        >
                                    ...
                                    <Progress value={displayDeal.aiProbability || displayDeal.probability} className="h-2" />
                                </div>
                                ...
                                {dealDetails?.activities?.length ? (
                                    ...
                                    <div className="flex items-start gap-3 p-3 border border-border/50 rounded-lg">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-muted-foreground">New deal created in the pipeline</p>
                                        </div>
                                    </div>
                                )}
            ...
            <EditDealDialog deal={displayDeal} open={editDialogOpen} onOpenChange={setEditDialogOpen} />
        </>
    )
}
